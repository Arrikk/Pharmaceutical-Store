$('#update-login').on('click', function(){
    ...
    axios.post('/api/account/update', {username:login, id}, { headers: bz.headers }).then(res => {
...
$('#update-password').on('click', function(){
    ...
    if(newPassword.val() != cPassword.val()){
        ...
        btn.html(`<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>`).attr('disabled', 'disabled')
        axios.post('/api/password/update', {
            oldPassword: oldPassword.val(),
            newPassword: newPassword.val()
        }, {headers: bz.headers}).then(res => {
